...
| Skills | 21 | `/boris` (default for non-trivial tasks), `/cross-review`, `/prime-agent`, `/loops`, `/session-start`, `/checks`, `/bspec-doc`, `/fix-issue`, `/drift-check`, `/handoff`, `/memory-migrate`, and more — same `/name` invocation, now with tool grants, argument hints, and invocation control |
