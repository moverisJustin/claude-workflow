...
| Rules | 5 | `git-safety.md`, `workflow.md` (always-on policy), `documentation-channels.md` (Linear + BSpec loop-closing contract), `writing.md` (the Brief block + decision-brief contract, plus the chat-stream rules for every answer Claude writes; enforced by `ste-check.sh`, and `ste-check.sh --chat` for a pasted answer), `learned-patterns.md` (the lesson-capture/sync target) — installed to `~/.claude/rules/` |
